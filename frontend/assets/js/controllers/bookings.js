...
import { resetForm, fillFrom } from "../components/BookingForm.js";
...
export function initBookingControllers() {
  ...
  $("bookingForm").addEventListener("submit", async (e) => {
     ...
     const data = {
      ...
      const { editingId } = getState();
       ...
       editingId
       ? await updateBooking(editingId, date)   // If editingId exists, update the bookings
       : await createNewBooking(data);  // Otherwise, create a new booking
  });
...
export async function updateBooking(id, data) {
    const res = await apiUpdatebBooking(id, data);
    if (res.ok) {
        showAlert("Updated!");
         resetForm();
        setState({ editingId: null });
